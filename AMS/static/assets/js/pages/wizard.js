$(document).ready(function() {
	...
	//------------- Create success msg if form is submit -------------//
 	function createSuccessMsg (loc, msg) {
		loc.append(
			'<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button><strong><i class="en-checkmark s24"></i> 已完成</strong> '+ msg + ' </div>'
		);
	}
	//------------- Form wizard with steps-------------//
 	// $("#wizard").formwizard({function(){
		...
		var dept_name = $("#dept_name").val()
                ...
                $.ajax({
                        ...
                        success: function (data) {
                            if (data.status == 'fail') {
                                if (data.msg == '内容为空或已存在') {
                                    alert('内容不能为空，不能存在相同的部门');
                                    window.location.href = "/users/add_dept/";
                                }
                                else {
                                    alert(data.msg)
                                }
	 	...
	 	// formOptions : {
			...
			// 	createSuccessMsg($("#wizard .msg"), "部门信息已提交");
			// 	setTimeout(function () {
			// 		window.location.reload();
          //       }, 1000);
         //
	...
	//------------- Form wizard without steps -------------//
 	$("#wizard1").formwizard({ 
	 	...
	 	formOptions :{
			success: function(data){
				//produce success message
				createSuccessMsg($("#wizard1 .msg"), "222You successfully22 submit this form222");
			},
			resetForm: true
	 	},
	 	disableUIStyles: true,
	...
	//------------- Vertical Form wizard with steps-------------//
 	$("#wizard2").formwizard({ 
	 	...
	 	formOptions :{
			success: function(data){
				//produce success message
				createSuccessMsg($("#wizard2 .msg"), "33You successfully33 submit this form333");
			},
			resetForm: false
	 	},
	 	disableUIStyles: true,
	...
	//------------- Wizard with validation -------------//
 	$("#wizard3").formwizard({ 
	 	...
	 	formOptions :{
			success: function(data){
				//produce success message
				createSuccessMsg($("#wizard2 .msg"), "44You successfully44 submit this form444");
			},
			resetForm: false
	 	},
	 	disableUIStyles: true,
